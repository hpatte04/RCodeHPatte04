...
##Pigeon, Abitbi and Nipigon are different, but represent a large portion of our sample in a geographically central location. Check if ecodistrics are responsible (line 927)


### Are there trends in the PD spring green-up value? How are they arranged geographically?
...
##Low and Hi are still significantly different  after amalgamation
...
### Are there trends in the IN and PT spring green-up value? How are they arranged geographically?
###Subset by Ecoregion IN
names(ANOVAReshapedMEDECOREGIONEQ8017292)[names(ANOVAReshapedMEDECOREGIONEQ8017292) == "ECOREGION_"] <- "ECOREGION"
levels(factor(ANOVAReshapedMEDECOREGIONEQ8017292$ECOREGION)) 

IN2 <- subset(ANOVAReshapedMEDECOREGIONEQ8017292, ANOVAReshapedMEDECOREGIONEQ8017292$TIME=='IN')

table(IN2$ECOREGION, IN2$SPRAYED)

IN3 <- subset(IN2, IN2$ECOREGION!='Agassiz Clay Plain' & IN2$ECOREGION!='Big Trout Lake' & IN2$ECOREGION!='Lake St. Joseph' & IN2$ECOREGION!='Georgian Bay')
FACTORMEANSECOREGIN<-ddply(IN3, .(ECOREGION), summarize, mean=mean(MED))
str(IN3)
MEDECOREGIONINALL <- qplot(ECOREGION, MED, data = IN3, geom = "boxplot", main = "MED by Ecoregion, All Sites IN") + coord_flip()
loadfonts(device = "win")
MEDECOREGIONINALL + theme(text = element_text(family = "serif")) + ggtitle("MED by Ecoregion, All Sites IN") + theme(plot.title = element_text(hjust = 0.5)) + aes(x=reorder(ECOREGION,-MED, na.rm = TRUE, y=Ecoregion)) + labs(x="EcoRegion", y="Median Spring Inflection Point Estimates (MED)") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))


kruskal.test(MED ~ ECOREGION, data = IN3)

DUNNTESTRESULTSPDALL<- data.frame

DUNNTESTRESULTSPDALL<-dunnTest(MED ~ ECOREGION, data = IN3, method = "holm")
write.table(DUNNTESTRESULTSPDALL[["res"]], file="C:/Users/Heather Patterson/Downloads/DUNNTESTRESULTSPDALLECOREGIONIN2.csv", sep=",", col.names=NA)

###Subset by Ecoregion PT and Spray
names(ANOVAReshapedMEDECOREGIONEQ8017292)[names(ANOVAReshapedMEDECOREGIONEQ8017292) == "ECOREGION_"] <- "ECOREGION"
levels(factor(ANOVAReshapedMEDECOREGIONEQ8017292$ECOREGION)) 

PT2 <- subset(ANOVAReshapedMEDECOREGIONEQ8017292, ANOVAReshapedMEDECOREGIONEQ8017292$TIME=='PT')
table(PT2$ECOREGION, PT2$SPRAYED)
PT3 <- subset(PT2, PT2$ECOREGION!='Agassiz Clay Plain' & PT2$ECOREGION!='Big Trout Lake')
PTTRUE <- subset(PT3, PT3$SPRAYED=='TRUE')
PTFALSE <- subset(PT3, PT3$SPRAYED=='FALSE')

kruskal.test(MED ~ ECOREGION, data = PTTRUE)
kruskal.test(MED ~ ECOREGION, data = PTFALSE)

##Hi Lo True
FACTORMEANSECOREGPTTRUE<-ddply(PTTRUE, .(ECOREGION), summarize, mean=mean(MED))
str(PTTRUE)
MEDECOREGIONPTTRUEALL <- qplot(ECOREGION, MED, data = PT3, geom = "boxplot", maPT = "MED by Ecoregion, All Sites PT Herbicide Treated") + coord_flip()
loadfonts(device = "wPT")
MEDECOREGIONPTTRUEALL + theme(text = element_text(family = "serif")) + ggtitle("MED by Ecoregion, All Sites PT Herbicide Treated") + theme(plot.title = element_text(hjust = 0.5)) + aes(x=reorder(ECOREGION,-MED, na.rm = TRUE, y=Ecoregion)) + labs(x="EcoRegion", y="Median SprPTg PTflection PoPTt Estimates (MED)") + theme(panel.grid.major = element_blank(), panel.grid.mPTor = element_blank(), panel.background = element_blank(), axis.lPTe = element_lPTe(colour = "black"))
...
DUNNTESTRESULTSPDALL<-dunnTest(MED ~ ECOREGION, data = PTTRUE, method = "holm")
write.table(DUNNTESTRESULTSPDALL[["res"]], file="C:/Users/Heather Patterson/Downloads/DUNNTESTRESULTSPDALLECOREGIONPTTRUE.csv", sep=",", col.names=NA)


FACTORMEANSECOREGPTFALSE<-ddply(PTFALSE, .(ECOREGION), summarize, mean=mean(MED))
str(PTFALSE)
MEDECOREGIONPTFALSEALL <- qplot(ECOREGION, MED, data = PT3, geom = "boxplot", maPT = "MED by Ecoregion, All Sites PT Herbicide Treated") + coord_flip()
loadfonts(device = "wPT")
MEDECOREGIONPTFALSEALL + theme(text = element_text(family = "serif")) + ggtitle("MED by Ecoregion, All Sites PT Herbicide Treated") + theme(plot.title = element_text(hjust = 0.5)) + aes(x=reorder(ECOREGION,-MED, na.rm = FALSE, y=Ecoregion)) + labs(x="EcoRegion", y="Median SprPTg PTflection PoPTt Estimates (MED)") + theme(panel.grid.major = element_blank(), panel.grid.mPTor = element_blank(), panel.background = element_blank(), axis.lPTe = element_lPTe(colour = "black"))


DUNNTESTRESULTSPDALL<- data.frame

DUNNTESTRESULTSPDALL<-dunnTest(MED ~ ECOREGION, data = PTFALSE, method = "holm")
write.table(DUNNTESTRESULTSPDALL[["res"]], file="C:/Users/Heather Patterson/Downloads/DUNNTESTRESULTSPDALLECOREGIONPTFALSE.csv", sep=",", col.names=NA)


write.table(ANOVAReshapedMEDECOREGIONEQ8017292, file="C:/Users/Heather Patterson/Downloads/PivotTable.csv", sep=",", col.names=NA)
